...
resource backendWebApp 'Microsoft.Web/sites@2022-09-01' = {
  name: appName
  location: location
  tags: {'azd-service-name': 'backendWebApp'} // Needed by AZD
  properties: {
    siteConfig: {
...
output backendAppName string = appName
output frontendAppName string = '${appName}-frontend'

output backendWebAppUri string = 'https://${backendWebApp.properties.defaultHostName}'
output frontendWebAppUri string = 'https://${frontendWebApp.properties.defaultHostName}'

// output connectionSettings array = map(concat(dbConnector.listConfigurations().configurations, vaultConnector.listConfigurations().configurations), config => config.name)
output backendWebAppLogStream string = format('https://portal.azure.com/#@/resource{0}/logStream', backendWebApp.id)
output backendWebAppSSH string = format('https://{0}.scm.azurewebsites.net/webssh/host', backendWebApp.name)
output backendWebAppConfig string = format('https://portal.azure.com/#@/resource{0}/environmentVariablesAppSettings', backendWebApp.id)
